...
def parse_news():
    ...
    # 정규식을 이용해 뉴스날짜 뽑아내기
    news_dts = [re.findall(r'\d{4}.\d{2}.\d{2}.', area.find('div', {'class':'news_info'}).get_text())[0] for area in area_list]
    ...
    # 딕셔너리 생성
    news = {}
    for idx, (n, content, news_dt) in enumerate(zip(a_list, contents, news_dts)):
        news[idx] = {'title' : n.get('title'),
                      'content' : content,
                      'url' : n.get('href'),
                     'release_date' : news_dt.rstrip('.').replace('.','-')
                     }
